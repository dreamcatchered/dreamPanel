...
def get_bot_info(bot_dir: Path) -> dict:
    """Получить информацию о боте (название, конфиг, размер)."""
    bot_name = bot_dir.name
    now = time.time()
    
    if bot_name in _bot_info_cache:
        cached = _bot_info_cache[bot_name]
    ...
    info = {
        'name': bot_name,
        'path': str(bot_dir),
        'has_config': (bot_dir / 'config.json').exists() or (bot_dir / 'settings.json').exists(),
        'has_env': (bot_dir / '.env').exists(),
        'entrypoints': [f.name for f in bot_dir.glob('*.py') if f.name != 'config.py'],
        'size_bytes': 0
    }
    
    # Считаем размер (только если нужно, можно ограничить глубину)
    try:
        total_size = 0
    ...
    _bot_info_cache[bot_name] = {'time': now, 'data': info}
    return info
